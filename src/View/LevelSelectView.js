...
LevelSelectView.prototype.drawLevelResults = function(value) {
    ...
    result += ' (' + number + ')';
    this.brush.fillText('Level ' + value, this.renderRegion.width/2, this.renderRegion.height * .10);
    this.brush.font = 'bold ' + Math.min(this.renderRegion.width * .25, 100) + 'px Arial';
    this.brush.fillText(result, this.renderRegion.width/2, this.renderRegion.height * .30);
    this.brush.font = Math.min(this.renderRegion.width * .1, 40) + 'px Arial';
};

LevelSelectView.prototype.drawDefaultView = function() {
    this.brush.font = 'bold ' + this.blockSize * .5 + 'px Arial';
    this.brush.fillText('Select a Level', this.renderRegion.width/2, this.renderRegion.height * .10);
};
